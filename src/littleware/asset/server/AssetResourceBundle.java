...
public class AssetResourceBundle extends ListResourceBundle {
    ...
    public enum Content {
        AssetDbManager, AssetRetriever, AssetManager,
        CacheConnection, AssetSearcher, CacheManager,
        CacheDbManager, AssetManagerServiceProvider,
        AssetSearchServiceProvider,
        AccountManager, AuthDbManager;
	...
	private static synchronized void initBundle () {
        ...
        try {
            ...
            prop_database = PropertiesLoader.loadProperties ( "littleware_jdbc.properties", 
                                                                         prop_database_default
                                                                         );
        } catch ( IOException e ) {
        ...
        try {
            ...
            Date   t_now = new Date ();
            String s_connection_url = "jdbc:derby:directory:" + prop_database.getProperty ( "derby_workarea" ) +
                "/" + t_now.getTime ();
            ...
            Connection          sqlconn_derby = DriverManager.getConnection( s_connection_url + 
                                                                             ";createFrom=" + s_derby_template
                                                                             );
            ...
            Content.CacheConnection.set ( new GuardedObject ( sqlconn_derby,
                                                              new AccessPermission( "generic" )
                                                              )
                                          );
            
            Content.CacheManager.set ( new GuardedObject ( SimpleCacheManager.getTheManager (),
                                                          new AccessPermission ( "cache_manager" )
                                                          )
                                      );
            Content.CacheDbManager.set ( new GuardedObject ( m_dbcache,
                                                            new AccessPermission ( "cache_dbmanager" )
                                                            )
                                        );
                                                                                           
            {
                final PrivilegedAction action_getresource = new GetGuardedResourceAction ( SqlResourceBundle.getBundle (),
			...
			Content.AssetRetriever.set ( new GuardedObject ( m_retriever,
													new AccessPermission ( "general" )
													)
                                          );
			Content.AssetManager.set ( om_asset );
            Content.AssetDbManager.set ( new GuardedObject ( m_dbasset,
                                                             new AccessPermission ( "asset_dbmanager" )
                                                             )
                                         );
            ...
            Content.AuthDbManager.set ( new GuardedObject ( m_dbauth,
                                                              new AccessPermission( "generic" )
                                                              )
                                          );
            
            Content.AssetManagerServiceProvider.set (
                                                     new AbstractServiceProviderFactory<AssetManager> ( ServiceType.ASSET_MANAGER, m_searcher ) {
                                                         public AssetManager createServiceProvider ( SessionHelper m_helper ) throws BaseException, AssetException, 
                                                         GeneralSecurityException, RemoteException
                                                     { 
                                                         AssetManager m_proxy = checkAccessMakeProxy ( m_helper, false,
                                                                                                       om_asset
                                                                                                       );
            ...
            Content.AssetSearchServiceProvider.set (
                                                    new AbstractServiceProviderFactory<AssetSearchManager> ( ServiceType.ASSET_SEARCH, m_searcher ) {
		...
		{
			String[]    v_always = {
				"littleware.asset.AssetType",
				"littleware.security.SecurityAssetType"
			};
		...
		try {
			...
			if ( null != s_user_classes ) {
				try {
					...
					for ( String s_token = null;
						  tok_classlist.hasMoreTokens ();
						  ) {
		...
		for ( String s_name : v_load_classes ) {
			try {
				...
				olog_generic.log ( Level.SEVERE, "Failed to load class: " + s_name + 
								   ", caught: " + e + " with cause: " + e.getCause () );
				throw e;
			}
		}        
	}
