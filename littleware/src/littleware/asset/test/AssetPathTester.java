...
public class AssetPathTester extends LittleTest {
    ...
    public void setUp() {
        try {
            ...
            Asset a_test = om_search.getAssetFrom(a_home.getId(), "AssetPathTester").getOr(null);
            if (null == a_test) {
                a_test = om_asset.saveAsset(
            ...
            Asset a_A = om_search.getAssetFrom(a_test.getId(), "A").getOr(null);
            Asset a_pointer = om_search.getAssetFrom(a_test.getId(), "Points2A").getOr(null);
            if (null == a_A) {
                a_A = om_asset.saveAsset(
                        AssetType.GENERIC.create().parent(a_test).
                        name("A").
                        comment("AssetPath traversal test area").build(), "Setting up AssetPathTester test area");

                if (null != a_pointer) {
                    a_pointer = om_asset.saveAsset(a_pointer.copy().toId(a_A.getId()).build(), "Update TO pointer for new A asset");
                }
            }
            if (null == a_pointer) {
                a_pointer = om_asset.saveAsset(
            ...
            for (int i = 1; i < 4; ++i) {
                ...
                Asset a_number = om_search.getAssetFrom(a_A.getId(), s_name).getOr(null);
                if (null == a_number) {
                    a_number = om_asset.saveAsset(
            ...
            Asset a_smallest = om_search.getAssetFrom(a_A.getId(), "smallest").getOr(null);
            if (null == a_smallest) {
                a_smallest = om_asset.saveAsset(
                        AssetType.LINK.create().
                        name("smallest").
                        parent(a_A).
            ...
            Asset a_biggest = om_search.getAssetFrom(a_A.getId(), "biggest").getOr(null);
            if (null == a_biggest) {
                a_biggest = om_asset.saveAsset(
                        AssetType.LINK.create().
                        name("biggest").
                        parent(a_A).
            ...
            olog_generic.log(Level.SEVERE, "Failed to setup AssetPathTester", e );
    ...
    public void testPathTraverse() {
        ...
        try {
            ...
            for (AssetPath path_test : v_tests) {
                ...
                AssetPath path_root = opathFactory.createPath(path_test.getRoot(om_search).get().getId(),
                ...
                AssetPath path_id_string = opathFactory.createPath(path_root.getRoot(om_search).get().getId(),
                        path_root.getSubRootPath());
                ...
                final AssetPath pathRoot = opathFactory.toRootedPath(opathFactory.createPath(a_test.getId()));
                assertTrue("Root path resolver works ok: " + pathRoot,
                        pathRoot.getAsset(om_search).equals(a_test) && pathRoot.toString().equals(opathFactory.toRootedPath(opathFactory.createPath(a_test.getId())).toString()));
                ...
                if (1 == i_count) {
                    ...
                    assertTrue("Get by id ok",
                            opathFactory.createPath(a_smallest.getId().toString()).getAsset(om_search).equals(a_smallest));
