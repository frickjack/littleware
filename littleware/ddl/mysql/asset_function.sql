...
--
delimiter //
CREATE FUNCTION littleware.getTypeId ( s_param_name VARCHAR(32) )
    RETURNS VARCHAR(32)
	BEGIN
	    DECLARE s_var_pk_type    VARCHAR(32);
	    SELECT s_id INTO s_var_pk_type
...
GRANT EXECUTE ON FUNCTION littleware.getTypeId TO littleware_user;
...
--
delimiter //
CREATE FUNCTION littleware.getPermissionId ( s_param_name VARCHAR(32) )
    RETURNS VARCHAR(32)
	BEGIN
        DECLARE s_var_pk_type    VARCHAR(32);
	    SELECT s_id INTO s_var_pk_type
...
GRANT EXECUTE ON FUNCTION littleware.getPermissionId TO littleware_user;
...
--
delimiter //
CREATE PROCEDURE littleware.copyTransactionHistory ( l_param_transaction BIGINT )
    BEGIN
...
GRANT EXECUTE ON PROCEDURE
        littleware.copyTransactionHistory
	TO littleware_user;
...
--
delimiter //
CREATE PROCEDURE littleware.saveAsset ( s_param_name VARCHAR(80),
                                        s_param_pk_type VARCHAR(32),
                                        s_param_from_asset VARCHAR(32),
                                        s_param_to_asset VARCHAR(32),
                                        s_param_caller VARCHAR(32),
                                        s_param_acl VARCHAR(32),
                                        s_param_data VARCHAR(1024),
                                        f_param_value NUMERIC(16,4),
							            s_param_comment VARCHAR(256),
                                        s_param_update VARCHAR(128),
									    s_param_home VARCHAR(32),
                                        t_var_start TIMESTAMP,
                                        t_var_stop TIMESTAMP,
										s_param_uuid VARCHAR(32),
                                        s_param_owner VARCHAR(32),
                                        l_param_transaction BIGINT,
                                        l_param_src INTEGER
													 )
    BEGIN
        DECLARE
           l_var_transaction   BIGINT;
		DECLARE s_var_pk_home_type  VARCHAR;
		DECLARE    i_tmp               INTEGER;
		DECLARE    t_var_stop          TIMESTAMP;
		DECLARE    t_var_start         TIMESTAMP;
        DECLARE    rec_tmp             RECORD;

	    DECLARE    s_var_pk_home_type = littleware.getTypeId ( 'littleware.home' );
		DECLARE    t_var_stop = littleware.stringToTimestamp ( s_param_stop );
		DECLARE    t_var_start = littleware.stringToTimestamp ( s_param_start );
