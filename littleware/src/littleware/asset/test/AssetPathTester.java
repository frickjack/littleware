    ...
    public void setUp() {
        try {
            ...
            for (int i = 1; i < 4; ++i) {
                ...
                Asset a_number = om_search.getAssetFrom(a_A.getObjectId(), s_name).getOr(null);
                if (null == a_number) {
        ...
        try {
            ...
            for (AssetPath path_test : v_tests) {
                ...
                assertTrue("Root path resolver works ok: " + pathRoot,
                        pathRoot.getAsset(om_search).equals(a_test) && pathRoot.toString().equals(opathFactory.toRootedPath(opathFactory.createPath(a_test.getObjectId())).toString()));
