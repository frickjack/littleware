...
public class SimpleSessionManager extends UnicastRemoteObject implements SessionManager  {
	...
	private final Map<UUID,WeakReference<SessionHelper>> ov_session_map = 
		new HashMap<UUID, WeakReference<SessionHelper>> ();
	 ...
	 */
	private SessionHelper setupNewHelper ( LittleSession a_session )  throws BaseException, AssetException, 
		GeneralSecurityException, RemoteException {
		Subject       j_caller = a_session.getSubject ( om_search );
		SessionHelper m_helper = new SimpleSessionHelper ( a_session.getObjectId (),
														   om_search, om_asset, this
														   );
		InvocationHandler  handler_helper = new SessionInvocationHandler 
			( j_caller, m_helper, ServiceType.SESSION_HELPER.getCallLogger (),
			  ServiceType.SESSION_HELPER.getCallSampler (),
			  m_helper
			  );
		...
		ov_session_map.put ( a_session.getObjectId (),
							 new WeakReference ( m_rmi )
							 );
		return m_rmi;
	}	
