        ...
        try {
            ...
            try {	
                if ( null == a_old_asset ) {
                    ...
                    if ( (a_asset.getTransactionCount () > 0) 
                         && (a_old_asset.getTransactionCount () > a_asset.getTransactionCount ())
                         ) {
                    ...
                    if ( ! o_old.isOwner ( p_caller ) ) {
                        ...
                        if ( ( (a_old_asset.getAclId () == null) 
                               && (a_asset.getAclId () != null) )
                             || (
                                 ! a_old_asset.getAclId ().equals ( a_asset.getAclId () )
                                 )
                             ) {
                ...
                if ( (null != a_asset.getFromId ())
                     && ( (null == a_old_asset)
                          || ( ! a_asset.getFromId ().equals ( a_old_asset.getFromId () ) )
                          )
                     ) {
                    ...
                    if ( ! o_from.isOwner ( p_caller ) ) {
                        ...
                        if ( (null == acl_from)
                             || (! acl_from.checkPermission ( p_caller, LittlePermission.WRITE ))
                             ) 
                        {
                            throw new AccessDeniedException ( "Caller " + p_caller +
                    ...
                    if ( (! a_from.getHomeId ().equals ( a_asset.getHomeId ())
                          && (! a_asset.getHomeId ().equals ( a_asset.getFromId () ))  // linking FROM HOME
                          ) 
                         ) {
