...
public class PropertiesLoader {
	 ...
	 */
	public static synchronized Properties loadProperties ( String s_name, 
						   ...
						   ) throws IOException 
	{
		...
		if ( null == prop_filedata ) {
			...
			try {
				...
				olog_generic.log ( Level.INFO, "Insufficient privileges to access System property: " + 
								   s_name + ", caught: " + e 
								   );
			}
			...
			if ( null == s_path ) {
				...
				try {
					...
					olog_generic.log ( Level.INFO, 
									   "Insufficient privileges to access System property: littleware.install, caught: " + 
									   e 
									   );
				}					
				if ( null != s_workdir ) {
					...
					if ( ! fh_props.exists () ) {
						olog_generic.log ( Level.FINE, "littleware.install path does not exist for " + 
										   fh_props.getAbsolutePath () );
						s_path = s_name;
					}
				} else {
			...
			try {
				...
				if ( fh_props.exists () ) {
					...
					try {
						...
						olog_generic.log ( Level.WARNING, "Failure loading properties file: " + s_path + 
										   ", caught: " + e
										   );
						throw e;
					} finally {
					...
					olog_generic.log ( Level.INFO, "Not loading properties file that does not exist: " +
									   s_path
									   );
				}
			} catch ( SecurityException e ) {
				olog_generic.log ( Level.WARNING, "Security constrained environment, may not access: " + 
								   s_path + ", caught: " + e );
			}
		}

		Properties prop_result = new Properties ( prop_defaults );
		for ( Enumeration r_i = prop_filedata.propertyNames ();
			  r_i.hasMoreElements ();
			  ) 
		{
