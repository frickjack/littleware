...
public abstract class DynamicEnum<T extends DynamicEnum> implements java.io.Serializable, Comparable<T> { 	
	...
	private void registerMemberIfNecessary( Class<T> c_class, Permission perm_join ) {
		...
		synchronized( mv_subtypes ) {
			...
			if ( null == x_data ) {
				olog_generic.log ( Level.INFO, "Registering new new DynamicEnum type: " +
								   c_class.getName ()
								   );
	...
	public boolean equals ( Object x_other ) {
		return ((null != x_other)
				&& (x_other instanceof DynamicEnum)
				&& ((DynamicEnum) x_other).getObjectId ().equals ( this.getObjectId () )
				);
	}
	 ...
	 */
	public int hashCode () { return getObjectId ().hashCode (); }
	...
	public Object readResolve() throws ObjectStreamException {
		try {
			...
			olog_generic.log ( Level.WARNING, "Deserialization of asset-type: " + this + 
							   ", caught unexpected: " + e );
			return this;
		}
	}
