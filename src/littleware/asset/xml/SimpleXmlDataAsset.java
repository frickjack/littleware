...
public abstract class SimpleXmlDataAsset extends SimpleAsset implements XmlDataAsset {	
		...
		try {
			for ( Method method_scan : v_methods ) {
				...
				if ( ann_xmlin != null ) {
					...
					if ( (ann_xmlin.attribute () != null)
						 && (! ann_xmlin.attribute ().equals ( "" ))
						 ) {
						ov_attribute_in.put ( ann_xmlin.attribute (), xml_reader );
					}
					if ( (ann_xmlin.element () != null)
						 && (! ann_xmlin.element ().equals ( "" ))
						 ) {
					...
					if ( ann_xmlout != null ) {
						...
						if ( (ann_xmlout.attribute () != null)
							 && (! ann_xmlout.attribute ().equals ( "" ))
							 ) {
							ov_attribute_out.put ( ann_xmlout.attribute (), xml_writer );
						} 
						if ( (ann_xmlout.element () != null)
							 && (! ann_xmlout.element ().equals ( "" ))
							 ) {
	...
	private class XmlDataHandler extends DefaultHandler {
		...
		throws SAXException
		{
			if ( os_namespace.equals ( s_namespace ) 
				 && s_simple.equals ( os_root )
