...
public abstract class SessionUtil {
	...
	public final static int MAX_REMOTE_RETRY = 3;
	...
	static {
		...
		try {
			...
			if ( (null != s_server_type) 
				 && s_server_type.equals ( "local" ) 
				 ) {
	 ...
	 */
	public static SessionManager getSessionManager ( String s_host, int i_port ) 
		...
		if ( i_port != 0 ) {
			...
			while ( true ) { // Retry on RemoteException
				try {
					...
					return new SessionManagerProxy ( m_session,
													 new URL ( "http://" + s_host + ":" + i_port + 
															   "/littleware/SessionManager" )
													 );
			...
			try {
				PrivilegedExceptionAction<SessionManager> act_setup = 
					new PrivilegedExceptionAction<SessionManager> () {
						public SessionManager run () throws Exception {
							...
							SubjectInvocationHandler<SessionManager> handler_session =
								new SubjectInvocationHandler<SessionManager> ( null, 
																				 m_session,
																				 olog_generic,
																				  new SimpleSampler ()
																						 );
				...
				olog_generic.log ( Level.SEVERE, "Failed setup, caught: " + e + ", with cause: " +
								   e.getException () + ", " + BaseException.getStackTrace ( e )
								   );
				throw new AssertionFailedException ( "Failed to setup SessionManager, caught: " + e +
												 ", with cause: " + e.getException (),
												 e.getException ()
												 );
			} catch ( Throwable e ) {
