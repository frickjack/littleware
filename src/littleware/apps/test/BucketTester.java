...
public class BucketTester extends TestCase {
	 ...
	 */
	public BucketTester ( String s_test_name, AssetManager m_asset,
                          AssetSearchManager m_search, BucketManager m_bucket ) {
		super( s_test_name );
        om_asset = m_asset;
        om_search = m_search;
		om_bucket = m_bucket;
	...
	public void setUp () {
		try {            
			...
			UUID  u_test_folder = om_search.getAssetIdsFrom ( u_home,
														  AssetType.GENERIC
														  ).get ( os_test_folder );
			if ( null == u_test_folder ) {
                ...
                a_folder.setAclId ( om_search.getByName ( littleware.security.AclManager.ACL_EVERYBODY_READ,
	...
	public void testBucket () {
		try {
            ...
            assertTrue ( "Bucket tracks right asset", 
                         bucket_test.getAssetId ().equals ( oa_test.getObjectId () )
                         );
            assertTrue ( "Bucket starts empty",
                         bucket_test.getPaths ().isEmpty ()
                         );
            ...
            assertTrue ( "Bucket has a path now",
                         bucket_test.getPaths ().contains ( "test1" )
                         );
            oa_test = om_bucket.renameFile ( oa_test, "test1", "test_rename", "bucket rename test" );
            bucket_test = om_bucket.getBucket ( oa_test.getObjectId () );

            assertTrue ( "Rename went ok",
                         bucket_test.getPaths ().contains ( "test_rename" )
                         && (bucket_test.getPaths ().size () == 1)
                         );
            oa_test = om_bucket.writeToBucket ( oa_test, "test1", os_test_data + os_test_data, 
                                                "write another test file to the bucket" 
                                                );
            bucket_test = om_bucket.getBucket ( oa_test.getObjectId () );
            
            assertTrue ( "Bucket with multiple files went ok",
                         bucket_test.getPaths ().contains ( "test_rename" )
                         && bucket_test.getPaths ().contains ( "test1" )
                         && (bucket_test.getPaths ().size () == 2)
                         );
            
            String s_data = om_bucket.readTextFromBucket ( oa_test.getObjectId (), "test_rename" );
            assertTrue ( "Got expected data from bucket: " + s_data,
                         s_data.equals ( os_test_data )
                         );
            oa_test = om_bucket.eraseFromBucket ( oa_test, "test_rename", "erase some test data" );
            bucket_test = om_bucket.getBucket ( oa_test.getObjectId () );
            
            assertTrue ( "Erase 1 went ok",
                         bucket_test.getPaths ().contains ( "test1" )
                         && (bucket_test.getPaths ().size () == 1)
                         );
