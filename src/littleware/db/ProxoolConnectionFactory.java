...
public class ProxoolConnectionFactory implements ConnectionFactory {
	private static Logger olog_generic = Logger.getLogger ( "littleware.db" );
	 ...
	 */
	public ProxoolConnectionFactory ( String s_driver,
									  String s_url,
									  String s_username,
									  String s_password,
									  String s_proxool_alias,
									  int    i_max_connections
									  ) throws LittleSqlException 
	{
		...
		*/
		...
		if ( (s_proxool_alias.indexOf ( "." ) >= 0)
			 || (s_proxool_alias.indexOf ( ":" ) >= 0)
			 ) 
		{
	...
	public synchronized Connection getConnection () throws SQLException {
		...
		if ( (null != othread_connection.get ()) 
			 && (! othread_connection.get ().isClosed ())
			 ) 
		{
			try {
				throw new Exception ( "Get stacktrace" );
			} catch ( Exception e ) {
				throw new LittleSqlException ( "Single thread attempting to checkout multiple connections from same pool: " +
		...
		if ( sql_conn == null) {
			olog_generic.log ( Level.WARNING, 
