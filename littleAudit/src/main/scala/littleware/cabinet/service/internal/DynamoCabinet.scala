 ...
 */
@inject.Singleton
class DynamoCabinet @inject.Inject() (gs:gson.Gson, config:DynamoCabinet.Config) extends CabinetService {
     ...
     */
    @guava.annotations.VisibleForTesting
    private[internal] def putThing(
        thing:DynamoCabinet.DynamoThing
    ):CompletionStage[Try[DynamoCabinet.DynamoThing]] = {
        ...
        dynamo.putItem(putRequest).thenApply(
                response => Success(thing).asInstanceOf[Try[DynamoCabinet.DynamoThing]]
            ).exceptionally(
                throwable => Failure[DynamoCabinet.DynamoThing](throwable)
            )
    }
    ...
    private[internal] def getThing(
        ...
        withDetails: Boolean,
        withPayload: Boolean
    ):CompletionStage[Try[Option[DynamoCabinet.DynamoThing]]] = {
        ...
        dynamo.getItem(getRequest).thenApply(
            response => Try(
                Option.when(response.hasItem())(response.item).map(
                    item => DynamoCabinet.DynamoThing(
                        item.get("PK").s(),
                        item.get("SK").s(),
                        item.get("Metadata").s(),
        ...
        ).exceptionally(throwable => Failure(throwable))
    }

    override def saveDoc(
...
object DynamoCabinet {
    ...
    private[internal] object DynamoThing {
        private [internal] final class Builder extends PropertyBuilder[DynamoThing] {
            val pk = new Property[String](null) withName "pk" withValidator notNullValidator
            val sk = new Property[String](null) withName "sk" withValidator notNullValidator
            val metadata = new Property[String](null) withName "metadata" withValidator notNullValidator
            val details = new OptionProperty[String]() withName "details" withValidator notNullValidator
            val payload = new OptionProperty[String]() withName "payload" withValidator notNullValidator
            ...
            override def build(): DynamoThing = {
                this.validate()
                DynamoThing(pk(), sk(), metadata(), details(), payload(), newVersion(), expirationUnixTime())
            }
        }
