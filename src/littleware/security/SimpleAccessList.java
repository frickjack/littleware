...
public class SimpleAccessList extends SimpleAsset implements LittleAcl {
	...
	public boolean checkPermission ( Principal p_user, Permission perm_access ) {
		if ( ov_negative_user_entries.containsKey ( p_user ) 
			...
			return false;
		} else if ( ov_positive_user_entries.containsKey ( p_user )
					&& ((AclEntry) ov_positive_user_entries.get ( p_user )).checkPermission ( perm_access )
					) {
			return true;
		} else {
			...
			for ( Iterator<Principal> r_i = ov_negative_group_entries.keySet ().iterator ();
				...
				if ( p_group.isMember ( p_user )
					 && ((AclEntry) ov_negative_group_entries.get ( p_group )).checkPermission ( perm_access )
					 ) {
					return false;
				}
			}
    ...
    private void mapSync ( Map<Principal,AclEntry> map_syncto,
        ...
        try {
            ...
            throw new AssertionFailedException ( "Caught unexpected", e );
        }
    }
