...
public class JAclEditor extends JGenericAssetEditor implements AssetEditor {
    ...
    private static final Logger log = Logger.getLogger(JAclEditor.class.getName());
    ...
    private void uiAddNewMember() {
        try {
            ...
            if (null == entry) {
                entryBuilder = aclEntryProvider.get().acl(acl_local).
                        principal(principal);
            } else {
            ...
            log.log(Level.INFO, "Failed adding new member", ex );
            JOptionPane.showMessageDialog(null, "Add member failed, caught: " + ex, "Add failed", JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void uiDeleteMembers() {
        try {
            ...
            log.log(Level.INFO, "Failed removing member", ex );
            JOptionPane.showMessageDialog(null, "Remove member failed, caught: " + ex, "Remove failed", JOptionPane.ERROR_MESSAGE);
        }
    ...
    private void uiSaveLocalChanges() {
        try {
            ...
            log.log(Level.INFO, "Failed save", ex );
            JOptionPane.showMessageDialog(null, "Save failed, caught: " + ex, "Save failed", JOptionPane.ERROR_MESSAGE);
        }
    }
    //----------------------
    ...
    private void addAclTab() {
        if (ob_built) {
            log.log(Level.WARNING, "Ignoring second call to single-call method");
            return;
        }
        ...
        Icon icon_acl = olib_icon.lookupIcon(LittleAcl.ACL_TYPE);
    ...
    private void updateTab() {
        ...
        if ((null == amodel_view)
            ...
            log.log(Level.FINE, "Non-acl model");
            return;
        }
